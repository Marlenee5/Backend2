...
// Si existe una cookie en el request, la extraemos y returnamos el token
...
passport.use('jwt-cookie', new JwtStrategy(
    ...
    async (payload, done) => {
        try {
            const user = await User.findById(payload.sub).lean(); // Busco el usuario por el id almacenado en el payload del token
            if (!user) return done(null, false); // Usuario no encontrado
            return done(null, { _id: user._id, email: user.email, role: user.role }) // Usuario encontrado
        } catch (err) { return done(err); }
    }
))
